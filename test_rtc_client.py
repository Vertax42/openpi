...
def test_rtc_client():
    ...
    broker = rtc_action_chunk_broker.RTCActionChunkBroker(
        ...
        action_queue_size_to_get_new_actions=5,
        rtc_enabled=True,
    ...
    try:
        ...
        # First call might block or wait until queue has item
        # The broker implementation waits up to 1.0s if queue is empty
        ...
        for i in range(50):
            ...
            try:
                ...
                if action is not None:
                    print(f"Step {i:02d}: Action executed. Shape: {action.shape} | First dim: {action[0]:.4f}")
                else:
                    print(f"Step {i:02d}: Warning: Got None action")
                    
            except RuntimeError as e:
                print(f"Step {i:02d}: Error: {e}")
                print("Queue might be empty because inference is slower than control loop.")
